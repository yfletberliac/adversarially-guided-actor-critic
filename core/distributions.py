...
from core.tf_layers import linear
...
class DiagGaussianProbabilityDistribution(ProbabilityDistribution):
    def __init__(self, flat):
        ...
        self.mean = mean
        # Clip logstd
        self.clipfrac = tf.reduce_mean(tf.cast(tf.greater(LOG_STD_MIN, logstd), tf.float32)) + \
                        tf.reduce_mean(tf.cast(tf.greater(logstd, LOG_STD_MAX), tf.float32))
